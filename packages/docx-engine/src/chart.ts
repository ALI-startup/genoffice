...
/** Read the display model of a chart part (word/charts/chartN.xml). */
...
/** Build a complete chart part (word/charts/chartN.xml) from data. */
...
/**
 * Patch cached texts/numbers of a chart part while keeping the structure — data references (c:f),
 * styling, layout — byte-identical.
 */
...
/**
 * Build a minimal but valid xlsx file containing one Sheet1 with the chart data (header row + data
 * rows).
 */
...
export async function patchChartWorkbookXlsxBase64(
  ...
  try {
    ...
    // Replace only Sheet1's sheetData in place, using inline strings so sharedStrings.xml (and
    // every other part: styles, extra sheets, defined names) survives untouched.
