...
function paragraphSlices(xml: string): ParaSlice[] {
  // depth-aware scan: a paragraph may legally nest more <w:p> inside embedded textbox content
  // (w:txbxContent), so a lazy open-to-first-close match would truncate the outer paragraph
  const out: ParaSlice[] = []
