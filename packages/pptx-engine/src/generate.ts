/** Phase 3.3 element-level patch generation — regenerating OOXML fragments for dirty elements. */
...
/**
 * Surgically patch the <a:pPr> of one <a:p>…</a:p> slice: only reorder groups we model that are
 * flagged dirty (lnSpc/spcBef/spcAft/bu*); non-dirty groups keep their original bytes and join the
 * reorder in schema order; unmodeled children like defRPr/tabLst stay in place (schema order puts
 * them after these groups, so still valid).
 */
...
/**
 * Element-level paragraph formatting patch: surgically patch pPr per <a:p> (run bytes untouched).
 */
...
function patchRunProps(runXml: string, run: TextRun): string {
  const attrPatch = (rprOpen: string): string => {
    ...
    // Underline: keep the original style (dbl/wavy… not collapsed to sng); on removal an existing u
    // becomes none, and no u is injected when there was none (keeping bytes).
...
/**
 * In-place patch between fills of the same kind, preserving unmodeled parameters: solid->solid
 * keeps the original color's alpha; grad->grad keeps gradFill attributes (rotWithShape/flip), child
 * nodes like tileRect, the original radial path (off-center fillToRect), and a:lin's scaled.
 */
 ...
 * fill='none' -> <a:noFill/>; hex -> <a:solidFill>.
 */
...
/**
 * In-place patch of an element's stroke (the <a:ln> direct child of spPr): stroke=null -> replace
 * the fill child with <a:noFill/> (explicit no stroke, overriding theme inheritance); otherwise
 * change the w attribute and replace the fill child with a solid color.
 */
...
/**
 * In-place patch of the <a:srcRect> inside a <p:pic>'s <p:blipFill>: srcRect=null removes the
 * srcRect (show the full image); otherwise writes the l/t/r/b attributes (1/1000 %).
 */
...
/**
 * In-place patch of a slide's bodyPrefix: replace/inject the <p:bg> under <p:cSld> with a solid
 * color.
 */
...
/**
 * In-place patch of the bodySuffix's <p:transition> (schema order: after clrMapOvr, before timing).
 */
...
/**
 * In-place patch of the bodySuffix's auto-advance time (PowerPoint's "advance slide after /
 * rehearse timings").
 */
 ...
 * the first <a:xfrm> inside spPr/grpSpPr; when absent (placeholder inheriting geometry), inject a
 * full <a:xfrm> after the spPr open tag.
 */
...
/**
 * Write back <a:normAutofit>'s fontScale/lnSpcReduction (thousandths of a percent, 62500 = 62.5%).
 */
