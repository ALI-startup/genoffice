/**
 * Shape animation (<p:timing>) read/write — the engine layer behind PowerPoint's "Animations" tab.
 */
...
export interface SlideAnimation {
  ...
  delayMs: number
  /**
   * Motion path (valid when effect='motionPath'): SVG subset M/L/C/Z, coordinates 0..1 relative to
   * slide width/height (same as OOXML animMotion's relative path).
   */
  motionPath?: string
...
/**
 * Grouping + absolute in-group start: onClick starts a new "click group"; withPrev start = previous
 * start + delay; afterPrev start = previous end + delay.
 */
function placeAnims(
...
/**
 * Overwrite-style rebuild that only touches the main sequence: the mainSeq childTnLst is replaced
 * with new effect groups; unmodeled siblings (interactiveSeq, media timelines, ...) keep their
 * original bytes; bldP is rewritten, other bld* kept.
 */
...
/**
 * Incremental <p:timing> patch: touches only the affected effect nodes; unmodeled sibling nodes
 * (interactive trigger sequences, text-level animations, media timelines, ...) keep their original
 * bytes.
 */
...
/** Set the slide's full animation list. */
