...
import { applyPdfEdits, extractPagesBytes, insertPdfBytes } from '../src/index.js'
import type { PdfEditRequest } from '../src/index.js'
...
describe('applyPdfEdits', () => {
  it('applies page rotation deltas on top of the existing rotation', async () => {
    const bytes = await makePdf([[100, 100]])
    const saved = await applyPdfEdits(bytes, request({ rotations: [{ pageIndex: 0, delta: 90 }] }))
  ...
  it('writes markup annotations with an appearance stream', async () => {
    ...
    const saved = await applyPdfEdits(
      bytes,
      request({
        markups: [
          {
  ...
  it('deletes pages by original index but never removes the last page', async () => {
    ...
    const saved = await applyPdfEdits(bytes, request({ deletedPages: [0, 2] }))
    const out = await PDFDocument.load(saved)
    expect(out.getPageCount()).toBe(1)
    expect(out.getPage(0).getWidth()).toBe(200)

    const savedAll = await applyPdfEdits(bytes, request({ deletedPages: [0, 1, 2] }))
  ...
  it('reorders pages by original index', async () => {
    ...
    const saved = await applyPdfEdits(bytes, request({ pageOrder: [2, 0, 1] }))
    const out = await PDFDocument.load(saved)
  ...
  it('applies the reorder when combined with deletions', async () => {
    ...
    const saved = await applyPdfEdits(bytes, request({ deletedPages: [1], pageOrder: [2, 0] }))
    const out = await PDFDocument.load(saved)
    expect(out.getPageCount()).toBe(2)
  ...
  it('skips the reorder when deleting every page leaves the guarded last page', async () => {
    ...
    const saved = await applyPdfEdits(bytes, request({ deletedPages: [0, 1], pageOrder: [1, 0] }))
    const out = await PDFDocument.load(saved)
    expect(out.getPageCount()).toBe(1)
  })
  ...
  it('rejects unknown form fields without producing bytes', async () => {
    const bytes = await makePdf([[100, 100]])
    await expect(
      applyPdfEdits(
        bytes,
        request({ formValues: [{ name: 'missing', kind: 'text', value: 'x' }] }),
      ),
    ).rejects.toThrow()
  })
})
