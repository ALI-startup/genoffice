 ...
 *     graph.
 *
 * The docx / pptx / xlsx / hwpx extractors have neither problem: they take bytes
 * and use jszip + fast-xml-parser (+ @samugen/docx-engine, which already runs in
 * the docs renderer), all of which run unmodified in a browser. This entry point
 * exposes exactly those, so a web host reuses the same extraction the path-based
 * entry point performs instead of growing a second implementation of it.
 *
