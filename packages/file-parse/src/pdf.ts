...
// pdfjs needs the standard_fonts data directory for non-embedded standard fonts (Helvetica etc.);
// under Node a filesystem path works (same usage as the official Node example).
...
/**
 * pdfjs's Node compat layer borrows DOMMatrix from the optional dep @napi-rs/canvas, and pdf.mjs
 * calls `new DOMMatrix()` at module top level — in the packaged build (no node_modules inside asar)
 * the require fails, so the import throws "DOMMatrix is not defined" and PDF attachment parsing
 * breaks entirely (not reproducible in dev since the dep happens to be present).
 */
...
export async function pdfToText(bytes: Uint8Array): Promise<string> {
  ...
  // Explicitly import the worker module (its top level registers globalThis.pdfjsWorker, which the
  // fake worker prefers) — otherwise pdfjs looks up pdf.worker.mjs by path at runtime, the file
  // isn't next to the bundled chunk, and it fails with "Setting up fake worker failed".
