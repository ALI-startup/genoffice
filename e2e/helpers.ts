 ...
 * userData dir (via SAMUGEN_USER_DATA) so runs never touch real settings
...
interface LaunchOptions {
  ...
  userDataDir?: string
  /** UI language override (SAMUGEN_LANG); defaults to English for stable assertions */
  lang?: string
...
export async function launchShell(options: LaunchOptions): Promise<LaunchedApp> {
  ...
  const userDataDir = options.userDataDir ?? (await mkdtemp(join(tmpdir(), 'samugen-e2e-')))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...hostEnv,
      SAMUGEN_USER_DATA: userDataDir,
      SAMUGEN_LANG: options.lang ?? 'en',
