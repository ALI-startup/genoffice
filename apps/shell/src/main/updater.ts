...
import { createI18n, getUiLang, htmlLang } from '@samugen/i18n'
 ...
 * itself is injected at build time via the SAMUGEN_UPDATE_URL env var and
 ...
 * release pipeline.
 *
 * Dev preview: SAMUGEN_FAKE_UPDATE=<version> in an unpacked run opens the
...
export function initAutoUpdater(getWindow: () => BrowserWindow | null): void {
  ...
  if (!app.isPackaged && process.env.SAMUGEN_FAKE_UPDATE) {
    initFakeUpdate(getWindow, process.env.SAMUGEN_FAKE_UPDATE)
    return
  }
