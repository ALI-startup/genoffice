...
import { createI18n, getUiLang, type Lang, normalizeLang, setUiLang } from '@samugen/i18n'
...
/** office/pdf formats extract text via @samugen/file-parse; images skip text
...
/** Attachment text extraction via @samugen/file-parse (docx/pdf/pptx/xlsx/plain text) */
...
export function registerSheetsIpc(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.writeWorkbookRecovery, async (event, input: unknown) => {
    ...
    try {
      ...
      await writeWorkbookTo({
        client: entry.client,
        fs: saveFs,
        session,
        request,
        targetPath: recoveryPathFor(session.path),
      })
      return { ok: true }
    } catch (error) {
...
async function prepareWorkbookForOpen(
  ...
  const directory = join(app.getPath('temp'), 'samugen-imports', randomUUID())
