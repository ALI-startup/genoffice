...
async function findPivotTablePathForCache(
  ...
  for (const relsPath of allPaths) {
    ...
    // Resolve the target relative to the rels file's directory rels path:
    // xl/pivotTables/_rels/pivotTable1.xml.rels Target: ../pivotCache/pivotCacheDefinition1.xml
...
function subtractAreas(outer: AreaRef, inner: AreaRef): AreaRef[] {
  // Only handles the case where inner is fully inside outer (which it always is here: old ⊆ new).
...
export async function applyPivotLayoutExpansions(
  ...
  for (const update of updates) {
    ...
    if (grows) {
      ...
      // Growth — check added cells for pre-existing content.
