...
export function applyCfRules(
  ...
  // Blocks whose cfRule carries an extLst are the base half of an x14 extension (linked via x14:id)
  // — kept verbatim and guarded below.
  ...
  // x14 rules and preserved blocks keep their priorities; new rules take the free values.
...
function dataBarRule(rule: Record<string, unknown>, priority: number, stopIfTrue: boolean): string {
  ...
  // The base schema stores one bar color; the negative color is x14-only and is dropped.
  return (
