//! The sidecar's request/response protocol, and the dispatcher every host shares.
//!
//! Moved out of `main.rs` unchanged when the engine gained a second host. The desktop
//! sidecar is a process reading line-delimited JSON off stdin; a browser calls the same
//! dispatcher through `wasm.rs`'s exports, with the JSON crossing linear memory instead of a
//! pipe. Neither host may interpret a command differently from the other, which is exactly
//! what one dispatcher guarantees and two would not.
//!
//! The transport is the only difference. Sessions, the recalc cache, the paths in every
//! request and the errors they can fail with are identical on both, because in a browser the
//! files live in a WASI filesystem the host fills in before the call.
...
/// Dispatch one request line. The transport hands the bytes over; this is the engine.
pub fn handle_line(
    line: &str,
    ...
    let result = match request.command {
        ...
        Command::ConvertWorkbook { path, target_path } => {
            crate::convert::convert_to_xlsx(&path, &target_path).and_then(|result| {
...
impl Response {
    ...
    pub fn from_error(request_id: String, error: SidecarError) -> Self {
    ...
    pub fn failure(request_id: String, code: &'static str, message: String) -> Self {
        Self {
            version: PROTOCOL_VERSION,
            request_id,
            ok: false,
