/**
 * Main-process i18n strings for GenOffice Slides (dialogs, native menus, export, autosave
 * prompts).
 *
 * The eleven labels the document operations return are *not* here: they are needed by both
 * hosts, so they live in src/shared/ops-i18n.ts, which a browser can import and this file's
 * home directory is not. `tmOps` below is the main process's binding of that dictionary.
 */
...
/** Operation labels with the current UI language, as `OpsTranslate` expects them. */
export const tmOps = (
  key: Parameters<typeof tOps>[1],
  params?: Parameters<typeof tOps>[2],
): string => tOps(getUiLang(), key, params)
